...
public class RewardNetworkImpl implements RewardNetwork {
	...
	public RewardConfirmation rewardAccountFor(Dining dining) {
		// findByCreditcard in a account repository by using the "dining" object's creditcard#
		Account account = accountRepository.findByCreditCard(dining.getCreditCardNumber());

		// findByMerchantNumber in a restaurantRepository by using the "dining" object's merchange #
		Restaurant restaurant = restaurantRepository.findByMerchantNumber(dining.getMerchantNumber());

		// Calculate benefits for the account by manipulating the dining objects
		MonetaryAmount amount = restaurant.calculateBenefitFor(account, dining);

		// Make the contribution to the account
		AccountContribution contribution = account.makeContribution(amount);

		// Update the repository
		accountRepository.updateBeneficiaries(account);

		// Record the confirmed reward
